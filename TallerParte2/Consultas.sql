...
CREATE OR REPLACE VIEW AVIONES_DISPONIBLES AS SELECT VUELOS_PROG.id ID_VUELO_PROGRAMADO,
    A.ID ID_AVION_ASIGNAR, A.NUM_ASIENTOS_CLASE_ECONOMICA+A.NUM_ASIENTOS_PRIMERA_CLASE TOTAL_ASIENTOS    
  FROM ITINERARIOS I
  JOIN VUELOS V
  ON V.ID = I.ID_VUELO
  JOIN rutas R
...
CREATE OR REPLACE PROCEDURE PROGRAMAR_TRIPULACION_VUELO (ID_ITINERARIO IN ITINERARIOS.ID%TYPE) AS
